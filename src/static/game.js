...
function render_map(pg, dt) {
    ...
    if (dt >= 0.85) {
        ...
        for (const loc of destroyed) {
            pg.quad(boardX + (loc[0] + 0.5) * size, boardY + (loc[1]) * size,
                boardX + (loc[0] + 1) * size, boardY + (loc[1] + 0.5) * size,
                boardX + (loc[0] + 0.5) * size, boardY + (loc[1] + 1) * size,
                boardX + (loc[0]) * size, boardY + (loc[1] + 0.5) * size);
        }
    }
    pg.pop();
}
